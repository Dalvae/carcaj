...
<section x-data="footnotes()" x-init="initialize()">
    ...
    <div class="relative" x-data="{ isOpen: false }" @click.away="isOpen = false">
        <button @click="isOpen = !isOpen" class="inline-flex p-2">
            <svg class="w-12 h-12 fill-red-600 hover:fill-gold-600 transition-colors duration-300" viewBox="0 0 24 24">
                <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/>
            </svg>
        </button>

        <!-- Menú desplegable de redes sociales -->
        <div x-show="isOpen" x-transition class="absolute left-full top-0 transform translate-x-2 bg-white p-4 rounded-lg shadow-lg flex items-center gap-4">
            <div class="relative after:content-[''] after:absolute after:top-1/2 after:right-full after:border-8 after:border-transparent after:border-r-white after:-translate-y-1/2">
                <!-- Facebook -->
                <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo get_permalink(); ?>" target="_blank" class="inline-flex p-2">
                    <svg class="w-10 h-10 fill-rose-400 hover:fill-gold-800 transition-colors duration-300">
                        <use xlink:href="#icon-facebook"></use>
                    </svg>
                </a>
                <!-- Instagram -->
                <a href="instagram://story?url=<?php echo urlencode(get_permalink()); ?>&media=<?php echo urlencode(get_the_post_thumbnail_url(get_the_ID(), 'large')); ?>" target="_blank" onclick="window.open('https://www.instagram.com', '_blank'); return false;" class="inline-flex p-2">
                    <svg class="w-10 h-10 fill-rosa hover:fill-gold-800 transition-colors duration-300">
